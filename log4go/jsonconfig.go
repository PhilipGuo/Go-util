...
var exepath, exepatherr = exec.LookPath(os.Args[0])
...
func jsonToFileLogWriter(filename string, ff *FileConfig) (*FileLogWriter, bool) {
	...
	if len(ff.Filename) > 0 {
		logfilename = ff.Filename
		// fmt.Println("filename: " + logfilename)
	}
	if !file.IsExist(logfilename) {
		_, err := file.Create(logfilename)
		dirpath := filepath.Dir(exepath) + "\\" + filepath.Dir(logfilename)
	...
	flw := NewFileLogWriter(logfilename, rotate)
